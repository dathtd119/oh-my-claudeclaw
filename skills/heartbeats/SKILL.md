...
description: View or modify heartbeat daemon settings. Use when users ask to enable/disable heartbeat, change heartbeat interval, set heartbeat prompt, configure Telegram integration, check daemon status, or reset settings. Trigger phrases include "turn on heartbeat", "turn off heartbeat", "enable heartbeat", "disable heartbeat", "change interval", "set interval", "heartbeat every", "heartbeat prompt", "configure telegram", "set telegram token", "show settings", "reset settings", "daemon config", "heartbeat settings".
---

# Heartbeat Config Skill

View or modify the claudeclaw heartbeat daemon settings. Use `$ARGUMENTS` to determine the action.
...
2. If confirmed, write the default settings:
   ...
   ```
3. Confirm the reset. Note: this does not delete cron jobs — use the cron-jobs skill for that.
